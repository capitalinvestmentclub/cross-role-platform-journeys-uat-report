...
function setup(query=''){
 const dom=new JSDOM(html,{url:'https://capitalinvestmentclub.github.io/cross-role-platform-journeys-uat-report/'+query,runScripts:'outside-only'});const w=dom.window,d=w.document;
...
test('publishes all cross-role journeys, six-size cells and findings',()=>{const x=setup();assert.equal(x.all('.finding-row').length,23);assert.equal(x.all('#scenarios tr').length,7);assert.equal(x.all('.cells span').length,42);assert.equal(x.$('#metric-total').textContent,'23');assert.equal(x.$('#metric-urgent').textContent,'15');assert.match(x.d.body.textContent,/7\/7 scenarios closed/);assert.match(x.d.body.textContent,/Final UAT score: 2\/10/);x.dom.window.close();});
