window.ADMIN_UAT = {
  cutoff: '2026-09-12 deployed Chrome cross-role journey campaign',
...
const journeyScenarioIds = new Set(['ADM-050','ADM-051','ADM-052','ADM-053','ADM-054','ADM-055','ADM-056']);
window.ADMIN_UAT.scenarios = window.ADMIN_UAT.scenarios.filter(([id]) => journeyScenarioIds.has(id));
window.ADMIN_UAT.findings = window.ADMIN_UAT.findings.filter(({scenario}) => journeyScenarioIds.has(scenario));
...
window.PR_REVIEW_DATA = {
  meta: {
    ...
    url: 'https://github.com/capitalinvestmentclub/cross-role-platform-journeys-uat-report',
    source: 'Cross-role journey UAT records in the capitalinvestmentclub/webapp project'
  },
  findings: window.ADMIN_UAT.findings.map((finding) => ({
    ...
    line: 1,
    sourceUrl: 'https://github.com/capitalinvestmentclub/cross-role-platform-journeys-uat-report',
    evidenceRun: `Private ${finding.scenario} browser run record`,
    evidenceRunUrl: 'https://github.com/capitalinvestmentclub/cross-role-platform-journeys-uat-report',
    evidenceUrl: '',
...
document.getElementById('gaps').innerHTML=[
  ...
  ['High-priority workflow defects','ADM-050–056','Eleven high-severity defects remain across evidence, finance, agreements, audit, moderation, privacy and recovery.'],
  ['Responsive retest contract','All 7 scenarios','Repeat every journey in Chrome at all six required viewport sizes after fixes.']
